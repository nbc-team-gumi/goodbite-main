...
public class WaitingService {
    ...
    public WaitingResponseDto createWaiting(UserCredentials user,
        ...
        RLock lock = redissonClient.getLock(//분산락 객체 생성
            "createWaitingLock:"
                //Redisson을 사용하여 레스토랑 ID 기반의 락을 생성 -> 특정 레스토랑에 대한 웨이팅 등록이 동시에 발생하지 않도록
        ...
        //try { ... } finally { lock.unlock(); } -> 락을 획득한 상태에서 웨이팅 등록 로직을 수행. 모든 작업이 끝난 후에는 반드시 락을 해제.
        try {
            Restaurant restaurant = restaurantRepository.findByIdOrThrow(
                postWaitingRequestDto.getRestaurantId());
            ...
            // 이 작업은 try 블록 내에서 예외가 발생하더라도 반드시 실행된다. 이렇게 함으로써 락을 획득한 후에는 반드시 락을 해제하여 다른 스레드가 락을 획득할 수 있도록 한다.
            // 만약 finally 블록이 없다면, 예외 발생 시 락이 해제되지 않아 다른 스레드가 영구적으로 락을 획득하지 못하게 될 수 있다.
            // 이를 방지하기 위해 항상 finally 블록에서 락을 해제해야한다.
            // 예외가 발생하더라도 반드시 실행되기 때문에, 락을 안전하게 해제할 수 있다.
            // 임계 구역 내에서 데이터베이스 작업을 수행하다가 예외가 발생해도, finally 블록이 실행되어 락이 해제된다.
            // 이를 통해 다른 스레드가 계속해서 락을 획득할 수 있도록 보장한다.
            lock.unlock();
        }
    }*/
